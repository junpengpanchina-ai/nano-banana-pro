...
export function GenerateClient({
  ...
  async function handleGenerate(e: React.FormEvent) {
    ...
    try {
      const data = await submitGenerateImage(
        ...
        imageSize,
        generationMode,
        generationMode === "image" ? referenceItems.map((r) => r.signedUrl) : undefined,
      );
      if (!data.ok) {
        setError(data.error);
  ...
  const promptOkForMode =
    generationMode === "text" ? prompt.trim().length >= 1 : prompt.trim().length >= 5;
          ...
          <div className="order-2 flex flex-col gap-6 rounded-2xl border border-zinc-800/90 bg-[#141210] p-6 shadow-2xl lg:order-1">
            ...
            <form className="flex flex-col gap-5" onSubmit={handleGenerate}>
              ...
              {generationMode === "image" ? (
                <div className="flex flex-col gap-3 rounded-xl border border-zinc-800/80 bg-[#121110]/60 p-4 ring-1 ring-zinc-800/40">
                  <div className="flex flex-wrap items-end justify-between gap-2">
                    <div className="min-w-0 flex-1">
                      <span className={labelCls}>图生图 · 文字与参考图</span>
                      <p className="mt-1.5 text-xs leading-relaxed text-zinc-500">
                        以下为<strong className="font-medium text-zinc-400">同一组输入</strong>
                        ，会一并提交：先用文字说明改动或风格（至少 5 个字）；需要时再在下方添加参考图，可不传图、仅按文字生成。
                      </p>
                    </div>
                  ...
                  <div className="border-t border-zinc-800/70 pt-3">
                    <p className="mb-2 text-xs text-zinc-500">
                      ...
                      选填 · 拖入或点击区域上传（最多 10 张）· JPG / PNG / WEBP，单张 ≤10MB
                    </p>
                    <label
                      onDragOver={(e) => {
                        e.preventDefault();
                ...
                <label className="flex flex-col gap-2" htmlFor="generate-prompt">
